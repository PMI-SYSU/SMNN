...
for hidden_shape in N_range:
    ...
    if hidden_shape==100:
        d_range=np.array([[0,7],[7,28]])
    else:
        d_range=np.array([[0,6],[6,22]])
    for d1,d2 in d_range:
        ...
        if d1==0:
            ax.text(np.exp(x[int((d2-d1)/6)]-0.5)+0.2,np.exp(yvals[int((d2-d1)/6)])*1.5-4.7,r'slope$\approx$'+'{:.2f}'.format(popt[0]),fontsize=12)
        else:
            ax.text(np.exp(x[int((d2-d1)/6)]-0.5)+2.5,np.exp(yvals[int((d2-d1)/6)])*1.5-5,r'slope$\approx$'+'{:.2f}'.format(popt[0]),fontsize=12)
        #R2
        ybar=np.sum(y)/len(y)
        ...
        b=[4,17]
        bb=[[-0.3,-1.5],[1.2,7]]
        ax.vlines(d_range[1,0],4,18,linestyle='--',linewidth=2,color=c[j])
        ax.vlines(d_range[1,1],4,18,linestyle='--',linewidth=2,color=c[j])
        ax.text(d_range[1,0]-bb[j][0],b[j],'${}$'.format(d_range[1,0]),fontsize=10,color=c[j])
        ax.text(d_range[1,1]-bb[j][1],b[j],'${}$'.format(d_range[1,1]),fontsize=10,color=c[j])
