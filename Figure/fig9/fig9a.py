...
hidden_shape=100
P=3
model=MDL_RNN_mnist(input_shape,hidden_shape,output_shape,P)
...
for k in k_range:
    ...
    model=MDL_RNN_mnist(input_shape,hidden_shape,output_shape,P,mdl=0)
    _=model.to(device)
    model.Win=Win
    model.Wout=Wout
    ...
    acc=[]
    for i in range(3):  
        acc.append(test(model,3000).cpu().numpy())
    Acc.append(acc)
pruning_rate=np.array(pruning_rate)
prune_acc=np.array(Acc)
fig, ax = plt.subplots(1, 1, figsize=(8, 5),dpi=200)
mean=np.array([np.mean(x) for x in prune_acc])
std=np.array([np.std(x) for x in prune_acc])
yerr = np.zeros([2,len(mean)])
yerr[0,:] = std
yerr[1,:] = std
ax.errorbar(pruning_rate*100,mean*100,yerr=yerr[:,:],ecolor='k',elinewidth=0.5,marker='.',\
